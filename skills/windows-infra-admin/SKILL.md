...
description: >
  Third person. Trigger phrases the agent will match. Use when the user asks to 'windows-infra-admin' or mentions 'windows-infra-admin'. Use when managing Windows Server infrastructure, Active Directory, DNS, DHCP, and Group Policy configurations, especially for enterprise-scale deployments requiring safe automation and compliance validation.
